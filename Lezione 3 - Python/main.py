...
create a structure containing 'activitySegments' with:
    #una lista di dizionari, un dizionario per ogni spostamento
[
{
'day':2017-11-01
'start time':10:18:40
...
#print(len(dati['timelineObjects']))

segm = []
for x in dati['timelineObjects']:
    if 'activitySegment' in x and 'distance' in x['activitySegment']: #l'elemento è davvero un activityseg?
        x = x['activitySegment'] #rinomino per comodità così uso la x per accedere al dizionario direttamente
        day = x['duration']['startTimestamp'].split('T')[0]
        start_time = x['duration']['startTimestamp'].split('T')[1].split('.')[0]
        end_time = x['duration']['endTimestamp'].split('T')[1].split('.')[0]
        path = [[d7(x['startLocation']['latitudeE7']),
                 d7(x['startLocation']['longitudeE7'])]] #path è una lista di liste ( qui metto le coordinate di partenza)
        if 'waypointPath' in x: #qui metto tutte le coordinate che attraversa
            for p in x['waypointPath']['waypoints']:
                path.append([d7(p['latE7']), d7(p['lngE7'])])
        path.append([d7(x['endLocation']['latitudeE7']), #coordinate di arrivo
